...
public class ServerSend
{
	...
	public static void SendTCPDataToSteamId(SteamId steamId, Packet packet)
	{
		...
		packet2.SetBytes(packet.CloneBytes());
		packet2.WriteLength();
		SteamPacketManager.SendPacket(steamId, packet2, ServerSend.TCPvariant, SteamPacketManager.NetworkChannel.ToClient);
	}
	...
	public static void SendStats()
	{
		using (Packet packet = new Packet(58))
		{
			...
			foreach (Client client2 in Server.clients.Values)
			{
				...
				{
					packet.Write(client2.player.id);
					foreach (string key in Player.allStats)
					{
						packet.Write(client2.player.stats[key]);
					}
				}
			}
			ServerSend.SendTCPDataToAll(packet);
