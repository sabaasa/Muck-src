...
public class GroundSwordAttack : MonoBehaviour
{
	private void Start()
	{
		...
		for (int i = 0; i < 2; i++)
		{
			Transform transform = Object.Instantiate<GameObject>(this.rollPrefab, base.transform.position, base.transform.rotation).transform;
		...
		for (int j = 0; j < 2; j++)
		{
			Transform transform2 = Object.Instantiate<GameObject>(this.rollPrefab, base.transform.position, base.transform.rotation).transform;
