...
public class MobServerDragon : MobServer
{
	...
	{
		if (Vector3.Angle((VectorExtensions.XZVector(Boat.Instance.transform.position) - VectorExtensions.XZVector(base.transform.position)).normalized, this.mob.transform.forward) > 80f)
		{
			return;
		}
	...
	private void GroundedBehaviour()
	{
		...
		{
			this.serverReadyToAttack = false;
			base.Invoke("GetReady", Random.Range(3.5f, 4.5f));
			this.currentAttacks = 0;
			return;
		}
