...
public abstract class Hitable : MonoBehaviour, SharedObject
{
	...
	public virtual int Damage(int newHp, int fromClient, int hitEffect, Vector3 pos)
	{
		...
		{
			Object.Instantiate<GameObject>(this.numberFx, pos, Quaternion.identity).GetComponent<HitNumber>().SetTextAndDir((float)num, normalized, (HitEffect)hitEffect);
		}
		this.hp = newHp;
		if (this.hp <= 0)
		{
	...
	{
		Object.Instantiate<GameObject>(this.destroyFx, base.transform.position, this.destroyFx.transform.rotation);
	}
