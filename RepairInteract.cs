...
public class RepairInteract : MonoBehaviour, Interactable, SharedObject
{
	private void Start()
	{
		...
		if (this.dontIncreaseWithPlayers)
		{
			num = 1f;
		}
		for (int i = 0; i < this.requirements.Length; i++)
		{
			this.requirements[i] = Object.Instantiate<InventoryItem>(this.requirements[i]);
	...
	{
		Object.Instantiate<GameObject>(this.repairFx, base.transform.position, Quaternion.identity);
		if (this.replace)
		{
			if (this.fixedObject)
			{
