...
public class CampSpawner : MonoBehaviour
{
	...
	private void Start()
	{
		...
		CampSpawner.woodmanRandomGen = new ConsistentRandom(GameManager.GetSeed() + ResourceManager.GetNextGenOffset());
		this.shrines = new Vector3[this.maxCaves];
		this.mapChunkSize = MapGenerator.mapChunkSize;
		this.worldScale *= this.worldEdgeBuffer;
		foreach (CampSpawner.WeightedSpawn weightedSpawn in this.structurePrefabs)
		{
	...
	public virtual void Process(GameObject newStructure, RaycastHit hit)
	{
		...
		component.houseSpawner = this.houses;
		component.MakeCamp(this.randomGen);
	}
	...
	public GameObject FindObjectToSpawn(CampSpawner.WeightedSpawn[] structurePrefabs, float totalWeight)
	{
	...
	public CampSpawner.WeightedSpawn[] structurePrefabs;
