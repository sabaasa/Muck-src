...
public class CraftingUI : InventoryExtensions
{
	...
	public override void UpdateCraftables()
	{
		for (int i = 0; i < this.cells.Count; i++)
		{
			...
			{
				Object.Destroy(this.cells[i].gameObject);
			}
		}
		...
		foreach (InventoryItem inventoryItem in this.tabs[this.tabSelected].items)
		{
			...
			{
				InventoryCell component = Object.Instantiate<GameObject>(this.cellPrefab, this.cellsParent).GetComponent<InventoryCell>();
				...
				{
					GameObject gameObject = Object.Instantiate<GameObject>(this.requirementPrefab);
