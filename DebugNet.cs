...
public class DebugNet : MonoBehaviour
{
	...
	private void Fps()
	{
		if (this.fpsOn || this.speedOn || this.pingOn || this.bandwidthOn)
		{
			...
			text += string.Format("\nOnly rock: {0}", GameManager.instance.onlyRock);
			text += string.Format("\nDamage taken by any players: {0}", GameManager.instance.damageTaken);
			text += string.Format("\nAny powerups picked up by any players: {0}", GameManager.instance.powerupsPickedup);
			this.fps.text = text;
			return;
		}
		if (this.fps.enabled)
