...
public class GameManager : MonoBehaviour
{
	...
	private void Start()
	{
		if (GameManager.gameSettings == null && NetworkController.Instance == null)
		{
			...
			{
				Object.Instantiate<GameObject>(this.zone, Vector3.zero, Quaternion.identity);
			}
			Server.InitializeServerPackets();
			LocalClient.InitializeClientData();
			Object.Instantiate<GameObject>(this.testGame);
	...
	public void SpawnPlayer(int id, string username, Color color, Vector3 position, float orientationY)
	{
		...
		{
			Object.Instantiate<GameObject>(this.localPlayerPrefab, position, Quaternion.Euler(0f, 0f, 0f));
			component = PlayerMovement.Instance.gameObject.GetComponent<PlayerManager>();
		}
		else
		{
			component = Object.Instantiate<GameObject>(this.playerPrefab, position, Quaternion.Euler(0f, orientationY, 0f)).GetComponent<PlayerManager>();
	...
	public void KillPlayer(int id, Vector3 pos)
	{
		...
		Object.Instantiate<GameObject>(this.playerRagdoll, pos, playerManager.transform.rotation).GetComponent<PlayerRagdoll>().SetRagdoll(id, -playerManager.transform.forward);
	}
	...
	public void SpawnGrave(Vector3 gravePos, int playerId, int graveObjectId)
	{
		...
		GraveInteract componentInChildren = Object.Instantiate<GameObject>(this.gravePrefab, gravePos, Quaternion.identity).GetComponentInChildren<GraveInteract>();
	...
	public void GameOver(int winnerId, float time = 4f)
	{
		...
		base.Invoke("ShowEndScreen", time);
		MusicController.Instance.StopSong(-1f);
	...
	public void MakeStats(Packet packet)
	{
		...
		for (int i = 0; i < num; i++)
		{
			this.stats[i] = new Dictionary<string, int>();
			this.stats[i].Add("Id", packet.ReadInt(true));
			foreach (string key in Player.allStats)
			{
				this.stats[i].Add(key, packet.ReadInt(true));
			}
		}
	}
